...
load_df = pd.read_excel('test.xls')
...
plt.title(kind[0] + "과 의 " + kind[1] + " 가격 변동")
plt.legend()
...
fig.suptitle(kind[0] + "과 의 " + kind[1] + " 기간 별 가격 정보 ")
plt.title(str(list(day_dict.keys())[0]) + " ~ " + str(list(day_dict.keys())[-1]))
